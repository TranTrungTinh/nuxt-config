<template>
  <div class="main user-layout-reset">
    ...
    <a-form id="formReset" ref="formReset" autocomplete="off" :form="form">
      ...
      <a-popover v-if="sent" placement="rightTop" trigger="click" :visible="state.passwordLevelChecked">
    ...
    handleSubmit() {
      ...
      validateFields({ force: true }, (err, values) => {
        if (!err) {
          ...
          this.$Amplify.Auth.forgotPasswordSubmit(username, code, password)
            .then(() => {
    ...
    resend(e) {
      ...
      validateFields(['username'], { force: true }, (err, values) => {
        if (!err) {
          ...
          this.$Amplify.Auth.forgotPassword(values.username)
            .then(() => {
              this.sent = true
